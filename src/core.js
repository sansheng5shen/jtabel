...
Calendar.prototype = {
    ...
    _renderDateItem: function(rDate) {
        var that = this, sdf = that.sdf, lg = that.lg, html = [];
    ...
    _getChtmlItem: function(rDate) {
        var that = this, sdf = that.sdf, html = [], st_time, et_time, days = sdf.getDays(rDate), min_time = sdf.getStime(that.minDate), max_time = sdf.getStime(that.maxDate);
    ...
    _fixedIe6: function() {
        ...
        if ($.browser.msie && $.browser.version === "6.0") {
            ...
            if (!that.iframeNode.length) {
                ...
                var iframeHtml = '<iframe id="frame_' + that.calendarId + '" ' + 'style="filter:alpha(opacity=20);position:absolute;' + "z-index:1;width:" + cw + "px;height:" + h + ";left:" + left + ";top:" + top + ';"></iframe>';
    ...
    _setPostion: function() {
        ...
        if (triggerInputEl.length && !that.parentContainer) {
            ...
            if (that.showPosition === "right") {
                left = tn_offset.left + parseInt(triggerInputEl.outerWidth(), 10) - that._getContainerWidth() - parseInt(that.containerNode.css("border-left-width"), 10) - parseInt(that.containerNode.css("paddingLeft"), 10) - parseInt(that.containerNode.css("paddingRight"), 10);
            } else {
                left = tn_offset.left;
            }
            that.containerNode.css({
