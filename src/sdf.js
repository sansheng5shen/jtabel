...
SDF.prototype = {
    ...
    checkDateFormater: function() {
        ...
        var y = parseInt(arrayDate[0], 10),
            m = parseInt(arrayDate[1], 10),
            d = parseInt(arrayDate[2], 10);
        //这个不是万年历
        if (y < 2000 || y > 2100) {
            return false;
        }
    ...
    isNatureWeek: function(r2Date, isStrict) {
        ...
        if (!_isStrict) {
            if ((this.getStime(et) - this.getStime(st)) / 864e5 <= 6) {
                return true;
            } else {
    ...
    hasNatureMonth: function(r2Date, dir, isStrict) {
        ...
        if (dir < 0) {
            ...
            //prevMonth 是该月的最后一天
            var prevMonth = this.getStepMonth(monthFirstDay, -1),
                prevMonthDays = this.getDays(prevMonth);
            ...
            var nextMonth = this.getStepMonth(monthLastDay, 1),
                nextMonthDays = this.getDays(nextMonth),
                nextMonthFirstDay = [this.getYears(nextMonth), this.getMonths(nextMonth), 1];
